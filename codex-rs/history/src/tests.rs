...
#[test]
/// Keeps the durable compaction join key and installed-context facts across rollout round trips.
fn compacted_item_serializes_durable_continuity_index() -> Result<()> {
    let continuity = CompactionContinuity {
        compaction_id: "compaction-7".to_string(),
    ...
    let serialized = serde_json::to_value(&item)?;
    assert_eq!(serialized["continuity"]["compaction_id"], "compaction-7");
    assert_eq!(serialized["continuity"]["phase"], "installed");
    assert_eq!(
        serialized["continuity"]["reference_context_installed"],
        true
    );
    assert_eq!(
        serde_json::from_value::<CompactedItem>(serialized)?.continuity,
        Some(continuity)
    );
    Ok(())
}
