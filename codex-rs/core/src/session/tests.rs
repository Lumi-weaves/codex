...
async fn user_shell_commands_do_not_inherit_managed_network_proxy() -> anyhow::Result<()> {
    ...
    let turn_context = session.new_default_turn().await;
    let managed_proxy = turn_context.network.as_ref().expect("managed proxy");
    let managed_proxy_url = managed_proxy
        .prepare_for_remote_environment(std::collections::HashMap::new(), "user-shell-test")?
        .env["HTTP_PROXY"]
        .clone();
    ...
    loop {
        ...
        if let EventMsg::ExecCommandEnd(event) = event.msg {
            ...
            assert_ne!(event.stdout.trim(), managed_proxy_url);
            break;
        }
    }
...
async fn late_completion_stays_queued_then_wakes_fresh_turn() {
    ...
    session
        ...
        .enqueue_pending_session_input(
            TurnInput::ResponseItem(ContextualUserFragment::into(completion).into()),
            /*trigger_turn*/ true,
            /*parent_turn_id*/ None,
        )
        .await;
...
async fn early_completion_drains_at_next_safe_boundary() {
    ...
    // next sampling input.
    let (items, _parent_turn_id, _root_turn_id) = session
        .input_queue
...
async fn later_turn_complete_with_zero_awaited_terminals_becomes_completed() {
    ...
    session
        ...
        .enqueue_pending_session_input(
            TurnInput::ResponseItem(
                ContextualUserFragment::into(crate::context::UnifiedExecCompletionEvent::new(
                    4249,
                    "sleep 10",
                    Some(0),
                    None,
    ...
    let (items, _parent_turn_id, _root_turn_id) =
        session.input_queue.drain_pending_session_inputs().await;
    assert_eq!(items.len(), 1);
    wait_for_session_event(&mut rx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    assert_eq!(
        session.agent_status.borrow().clone(),
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    let (_session, _turn_context) = make_session_and_context().await;
    let (tx_sub, rx_sub) = async_channel::bounded::<super::SessionIngress>(4);
    let (_tx_event, rx_event) = async_channel::unbounded();
...
async fn shutdown_and_wait_shuts_down_cached_guardian_subagent() {
    ...
    let (child_tx_sub, child_rx_sub) = async_channel::bounded::<super::SessionIngress>(4);
