...
///
/// Like trigger-turn mailbox work, terminal completions deliberately wake idle
/// sessions in Plan mode too. This is the fixed external-event policy:
/// wake-if-idle and queue-if-busy.
    ...
    match event {
            ...
            sess.input_queue
                .enqueue_pending_session_input(
                    TurnInput::ResponseItem(
                        crate::context::ContextualUserFragment::into(completion).into(),
                    ),
                    /*trigger_turn*/ true,
                    /*parent_turn_id*/ None,
                )
        ...
        super::internal_event::InternalSessionEvent::UnifiedExecOutputAvailable(attention) => {
            sess.input_queue
                .enqueue_pending_session_input(
                    TurnInput::ResponseItem(
                        crate::context::ContextualUserFragment::into(attention).into(),
                    ),
                    /*trigger_turn*/ true,
                    /*parent_turn_id*/ None,
                )
                ...
                let should_exit = async {
                    match sub.op {
                        ...
                        Op::RecoverTurn {
                            ...
                            let result =
                                turn_input::handle_recovery(&sess, thread_settings, sub.id.clone())
                                    .await;
                            let _ = reply.send(result);
                            false
                        }
