    ...
    match id {
        ...
        PromptContributionKind::CodexAgentBaseInstructions => StaticSourceNavigation {
            modules: &[
                ...
                "codex-rs/core/src/agent_program.rs",
                "codex-rs/core/src/prompt_agent_program_receipt.rs",
...
pub(crate) fn resource_definition(id: PromptContributionKind) -> StaticPromptResource {
    ...
    let (
        ...
        PromptContributionKind::CodexAgentBaseInstructions => (
            ...
            "logical Agent base instructions; Responses instructions or one leading developer item under Responses Lite",
            "codex@1-owned pinned reference to the gpt-5.6-sol legacy template during the strangler migration",
            "statically enumerable and runtime-accounted in prompt-receipt for explicitly selected codex@1 sessions",
            "no independent uniform bound; participates in the model context window",
            "versioned Agent resource; model identity must not select it",
            "pinned by Agent Definition revision and inherited by resume/fork Agent selection",
