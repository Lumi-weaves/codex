...
- `providerAccount/login/start`, `providerAccount/login/status`, and `providerAccount/login/cancel` (experimental) — run a multi-account OpenAI device login owned entirely by the bundled backend. Supplying an existing OAuth `accountId` reauthenticates that account in place and refuses a different upstream identity. The public lifecycle carries only an opaque login handle, verification URL/code, static failure state, and the completed secret-free account; authorization codes, PKCE material, tokens, and upstream account identity never cross app-server.
