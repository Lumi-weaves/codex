...
- `model/list` — list available models (set `includeHidden: true` to include entries with `hidden: true`), with an opaque semantic `revision`, revision-bound opaque pagination cursors, model-advertised string reasoning effort options in the catalog's intended progression order, optional `modelSpecialty`, nullable `multiAgentVersion` (`disabled`, `v1`, or `v2`), `additionalSpeedTiers`, `serviceTiers`, optional `defaultServiceTier`, optional legacy `upgrade` model ids, optional `upgradeInfo` metadata (`model`, `upgradeCopy`, `modelLink`, `migrationMarkdown`), and optional `availabilityNux` metadata. Clients should preserve the `supportedReasoningEfforts` array order rather than deriving order from the effort names. If the catalog changes between pages, the old cursor is rejected and the client should restart from the first page.
- `model/list/updated` — lightweight invalidation emitted when the effective model picker catalog changes semantically. It carries the new opaque `revision`; clients should invalidate cached picker data and refetch `model/list` rather than treating the notification as the catalog.
- `providerAccount/list` (experimental) — page through secret-free accounts and provider summaries owned by the bundled RichCodex provider plane. Revisions are opaque strings for cache comparison.
- `providerAccount/import` (experimental) — import one explicitly selected Codex `auth.json` into the RichCodex-owned credential store. App-server forwards the absolute path to the bundled backend and never opens the credential file itself.
- `providerAccount/apiKey/add` (experimental) — add one write-only API-key credential for OpenAI or a named OpenAI-compatible Responses API provider. The request supplies the provider's stable ID, display name, and safe API base URL; later accounts using the same ID must preserve that provider definition. Public responses expose only the opaque account handle, provider ID, label, credential kind, and lifecycle state; the key is never returned.
- `providerAccount/apiKey/replace` (experimental) — replace one API-key credential at an expected revision without changing its opaque account handle or any model target.
- `providerAccount/rename` (experimental) — change only a provider account's user-owned display label at an expected revision. The opaque account handle, credential, provider identity, and every model target remain unchanged.
- `providerAccount/removalPreview` and `providerAccount/remove` (experimental) — preview every affected tag/target, then remove only an unreferenced account at the previewed revision. Removal never rewrites targets implicitly.
- `providerAccount/login/start`, `providerAccount/login/status`, and `providerAccount/login/cancel` (experimental) — run a multi-account OpenAI browser-PKCE or device-code login owned entirely by the bundled backend. `mode` is `browser` for a local loopback callback or `deviceCode` for remote/headless use. Supplying an existing OAuth `accountId` reauthenticates that account in place and refuses a different upstream identity. The public lifecycle carries only an opaque login handle, an actionable authorization URL, an optional device code, static failure state, and the completed secret-free account; callback codes, PKCE verifier/state validation, tokens, and upstream account identity never cross app-server.
- `model/executionReceipt` (experimental) — emitted for a RichCodex-backed turn with its requested stable tag, resolved provider/model, opaque account and target handles, and pre-output route attempt number. When available, `clientAttemptId` is the opaque identity of the concrete Codex dispatch echoed by the private transport; it is separate from the backend route attempt. The receipt is persisted with the rollout and contains no credential or upstream account identity.
- `modelRoute/read` (experimental) — read the RichCodex-owned display-name → stable-tag → ordered-target projection. Account identifiers are opaque local handles; credentials never cross this API.
- `modelRoute/targets/set` (experimental) — atomically replace one stable tag's complete ordered target list at an expected model-plane revision. Existing opaque target handles may be preserved; omitted handles allocate new targets.
- `modelRoute/create` (experimental) — create one display entry, stable semantic tag, and initial account-bound provider target using optimistic concurrency via the opaque `expectedRevision` returned by a prior model-plane response.
- `modelRoute/retire` (experimental) — remove a display entry from the active picker projection without deleting its tag, targets, or provider-account binding. Mutations share one serialization scope and revision stream with provider-account imports.
- Model catalog overlays are republished after a successful `config/batchWrite` with `reloadUserConfig: true`. After editing the external `model_catalog_overlay_json` file directly, clients can send an empty `edits` array with that flag to request the same validated reload; an invalid overlay leaves the last valid runtime catalog published.
