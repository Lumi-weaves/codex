...
- `model/executionReceipt` (experimental) — emitted for a RichCodex-backed turn with its requested stable tag, resolved provider/model, opaque account and target handles, and pre-output attempt number. The receipt is persisted with the rollout and contains no credential or upstream account identity.
