...
- `modelWorkbench/read` — experimental read of OpenCodex's credential-free display-entry registry. The app-server uses a protected, process-scoped local capability; no OpenCodex admin or data-plane credential enters the RPC payload.
- `modelWorkbench/upsert` — experimental exact-tag display-name upsert with an optional `expectedRevision`. The response distinguishes the durable registry revision from catalog/cache publication and reports `pending` or `failed` instead of claiming that a `202 Accepted` backend receipt is fully synchronized.
- `modelWorkbench/retire` — experimental non-cascading retirement of one exact model tag. Retirement affects future picker discovery only; provider routes, accounts, credentials, and already-bound tasks remain owned by OpenCodex.
- After an accepted Model Workbench mutation, app-server reloads its configured authoritative catalog and overlay, then emits `model/list/updated` when the effective picker catalog changes, so connected clients can refetch without restarting app-server.
