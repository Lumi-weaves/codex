...
- `providerAccount/apiKey/add` (experimental) — add one write-only OpenAI API-key credential to the RichCodex-owned provider plane. Public responses expose only its opaque account handle, label, credential kind, and lifecycle state; the key is never returned.
