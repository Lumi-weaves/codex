...
#[tokio::test]
async fn thread_fork_keeps_model_routes_live_after_copying_effective_provider() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
...
"#,
    ...
    let fork_id = mcp
        .send_thread_fork_request(ThreadForkParams {
            thread_id: thread.id,
            model: Some("routed-model-a".to_string()),
            // Mirrors a copied source-thread effective provider. It remains a
            // fallback rather than pinning later routed models.
            model_provider: Some("routed_provider_a".to_string()),
            ..Default::default()
        })
        .await?;
    let forked: ThreadForkResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(fork_id)).await??;
    assert_eq!(forked.model, "routed-model-a");
    assert_eq!(forked.model_provider, "routed_provider_a");
